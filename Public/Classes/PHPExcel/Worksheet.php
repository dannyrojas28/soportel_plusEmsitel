    ...
    public function setTitle($pValue = 'Worksheet', $updateFormulaCellReferences = true)
    {
        ...
        if ($this->parent) {
            ...
            if ($this->parent->sheetNameExists($pValue)) {
                ...
                if (PHPExcel_Shared_String::CountCharacters($pValue) > 129) {
                    $pValue = PHPExcel_Shared_String::Substring($pValue, 0, 129);
                }
                $i = 1;
                while ($this->parent->sheetNameExists($pValue . ' ' . $i)) {
                    ++$i;
                    if ($i == 10) {
                        if (PHPExcel_Shared_String::CountCharacters($pValue) > 128) {
                            $pValue = PHPExcel_Shared_String::Substring($pValue, 0, 128);
                        }
                    } elseif ($i == 100) {
                        if (PHPExcel_Shared_String::CountCharacters($pValue) > 127) {
                            $pValue = PHPExcel_Shared_String::Substring($pValue, 0, 127);
                        }
                    }
                }
